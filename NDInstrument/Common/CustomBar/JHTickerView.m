...
//	[self.layer setBorderColor:[UIColor blackColor].CGColor];
	...
	// Set the font
	tickerFont = [UIFont boldSystemFontOfSize:18];
...
-(void)animateCurrentTickerString
{
    ...
    if (tickerStrings.count>0)
    {
        currentString = [tickerStrings objectAtIndex:currentIndex];
    }
    if (tickerString)
    {
        currentString = tickerString;
    }
    ...
    CGSize textSize = [NSString getSizeWithText:currentString andFont:tickerFont andSize:CGSizeMake(9999, self.frame.size.height) andSpacing:0];
